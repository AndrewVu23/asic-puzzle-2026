...
module puzzle (
    ...
    wire [3:0] lastch = success ? 4'd14 :         // "(* TWO STARS *)"   15
    ...
    always @(posedge clk or negedge rst_n)
        if (!rst_n) begin
            ...
            emitted  <= 1'b0;
            chidx    <= 4'd0;
        end else if (grid_done & ~emitted) begin
            emitting <= 1'b1;
            emitted  <= 1'b1;
            chidx    <= 4'd0;
        end else if (emitting) begin
            if (chidx == lastch) emitting <= 1'b0;
            else                 chidx    <= chidx + 4'd1;
        end

    reg [7:0] ch;
    always @* begin
        if (success)    // "(* TWO STARS *)"
            case (chidx)
